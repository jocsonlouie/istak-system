  ...
  <div>
    ...
    <div class="upper-btns mb-5 d-flex">
      <v-row no-gutters class="d-flex justify-center align-center">
        <v-col cols="12" sm="5" md="3" class="pa-1" v-if="isNonInventoryStaff">
          <v-dialog v-model="addDialog" persistent max-width="500">
            ...
            <v-card class="d-flex justify-center flex-column pa-md-10">
              <v-chip color="primary" class="d-flex justify-center font-weight-bold text-h6 pa-5 mb-4">New Inventory
              </v-chip>
              <v-img :src="itemImage" height="150px" class="rounded mb-4"></v-img>
              <v-form ref="form">
                <div class="text-center">
                  <v-btn color="primary" class="me-3 mt-2" @click="$refs.refInputEl.click()">
                  ...
                  <input ref="refInputEl" @change="uploadAvatar" type="file" accept=".jpeg,.png,.jpg,GIF" :hidden="true"
                    :rules="fieldRules" />
                ...
                <v-text-field label="Inventory Name" :rules="fieldRules" clearable filled class="mb-4"
                  v-model="inventory_name"></v-text-field>
              </v-form>
          ...
          <v-dialog v-model="updateDialog" persistent max-width="500">
            <v-card class="d-flex justify-center flex-column pa-md-10">
              ...
              <v-img :src="itemImageEdit" height="150px" class="rounded mb-4"></v-img>
              <v-form ref="editForm">
                <div class="text-center">
                  <v-btn color="primary" class="me-3 mt-2" @click="$refs.refInputElEdit.click()">
                    <v-icon class="d-sm-none">
                      {{ uploadBtnTextMobile }}
                    </v-icon>
                    <span class="d-none d-sm-block">{{ uploadBtnText }}</span>
                  ...
                  <input ref="refInputElEdit" @change="uploadAvatar" type="file" accept=".jpeg,.png,.jpg,GIF"
                    :hidden="true" :rules="fieldRules" />
                ...
                <v-text-field label="Inventory Name" :rules="fieldRules" clearable filled class="mb-4"
                  v-model="edit_inventory_name"></v-text-field>
              </v-form>
        ...
        <v-col cols="12" sm="6" md="3" class="pa-1">
          <v-card class=" bg-primary d-flex flex-column pa-4 justify-center align-center" height="200"
    ...
    <div class="inventories">
      <v-row no-gutters class="d-flex justify-start align-center">
        <v-col cols="13" xs="16" sm="6" md="3" class="pa-1" v-for="inventory in custom_inventories"
          :key="inventory.name">
          <!--  @click="gotoInventory" -->
          <v-card @click="gotoInventory(inventory.id)">
            ...
            <v-menu bottom left>
              <template v-slot:activator="{ on, attrs }" v-if="isNonInventoryStaff">
                <v-btn icon v-bind="attrs" v-on="on" absolute right top
              ...
              <v-list>
                <v-list-item v-if="isInventoryStaff">
                  <v-btn color="error" outlined small block class="me-2 mb-2" @click="
                ...
                <v-list-item>
                  <v-btn color="primary" outlined small block @click="
            ...
            <v-card-subtitle>{{ inventory.totalitems == null ? "0" : inventory.totalitems }}
              items,
...
<script>
  ...
  export default {
    data: () => ({
      ...
      itemImage: "https://firebasestorage.googleapis.com/v0/b/istak-2208e.appspot.com/o/custom-inventories%2Fbanner.jpg?alt=media&token=ce4dbfd6-25b5-4483-894c-f6855cd60cb8",
      itemImageEdit: "",
      //icons
